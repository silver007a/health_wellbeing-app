...
from repositories import goal_repository, smash_repository
from models.smash import Smash
from helpers.serializers import *
from flask import Blueprint

smash_blueprint = Blueprint("smash", __name__)

@smash_blueprint.route('/smashs')
def all_smashs():
    smashs = smash_repository.select_all()
    smashs_json = []
    for smash in smashs:
        serialized_smash = serialize_smash(smash)
        smashs_json.append(serialized_smash)
    return jsonify(smashs_json)
